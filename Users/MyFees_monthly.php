...
$rsStDetail=mysqli_query($Con, "select `sname`,`sclass`,`sfathername`,`MotherName`,`smobile`,`DiscontType`,`RouteForFees`,`MasterClass` ,`FinancialYear` from `student_master` where `sadmission`='$sadmission'");
	...
	//exit();
	$rsPreviousQuaterPaid=mysqli_query($Con, "select `quarter`,`BalanceAmt`,`receipt`,`FeeMonth` from `fees` where `sadmission`='$sadmission' and `cheque_status`!='Bounce' and `FinancialYear`='$FeeCollectionFY' order by `datetime` desc");
	$rowB=mysqli_fetch_row($rsPreviousQuaterPaid);
	$PreviousQuartePaid=$rowB[0];
	$PreviousBalance=$rowB[1];
	$LastPaymentReceiptNo=$rowB[2];
	$PreviousMonthPaid=$rowB[3];
	...
	{
		if(strpos($PreviousMonthPaid, 'April')!== false)
		{
		$ListOfQuarter=Q1;
		$ListOfMonth=May;
		}
		else if(strpos($PreviousMonthPaid, 'May')!== false)
		{
		$ListOfQuarter=Q1;
		$ListOfMonth=June;
		}
		else if(strpos($PreviousMonthPaid, 'June')!== false)
		{
		$ListOfQuarter=Q2;
		$ListOfMonth=July;
		}
	...
	{
	  	if(strpos($PreviousMonthPaid, 'July')!== false)
		{
		$ListOfQuarter=Q2;
		$ListOfMonth=August;
		}
		else if(strpos($PreviousMonthPaid, 'August')!== false)
		{
		$ListOfQuarter=Q2;
		$ListOfMonth=September;
		}
		else if(strpos($PreviousMonthPaid, 'September')!== false)
		{
		$ListOfQuarter=Q3;
		$ListOfMonth=October;
		}
	...
	{
	 	if(strpos($PreviousMonthPaid, 'October')!== false)
		{
		$ListOfQuarter=Q3;
		$ListOfMonth=November;
		}
		else if(strpos($PreviousMonthPaid, 'November')!== false)
		{
		$ListOfQuarter=Q3;
		$ListOfMonth=December;
		}
		else if(strpos($PreviousMonthPaid, 'December')!== false)
		{
		$ListOfQuarter=Q4;
		$ListOfMonth=January;
		}
 	 ...
 	 if(strpos($PreviousQuartePaid,'Q4') !== false)
	{
	  
	  	if(strpos($PreviousMonthPaid, 'January')!== false)
		{
		$ListOfQuarter=Q4;
		$ListOfMonth=February;
		}
		else if(strpos($PreviousMonthPaid, 'February')!== false)
		{
		$ListOfQuarter=Q4;
		$ListOfMonth=March;
		}
		else if(strpos($PreviousMonthPaid, 'March')!== false)
		{
		$ListOfQuarter=Q1;
		$ListOfMonth=April;
		}
	...
	$SelectedMonth=$ListOfMonth;
		...
		if (strpos($SelectedQuarter, 'Q1') !== false) 
			{
				$rsLastDt=mysqli_query($Con, "select FeesSubmissionLastDate from fees_master where Quarter='Q1'  and `Month`='$SelectedMonth' and  financialyear='$FeeCollectionFY' limit 1");
		...
		if (strpos($SelectedQuarter, 'Q2') !== false) 
			{
				$rsLastDt=mysqli_query($Con, "select FeesSubmissionLastDate from fees_master where Quarter='Q2' and `Month`='$SelectedMonth' and financialyear='$FeeCollectionFY' limit 1");
				$rowLastDt=mysqli_fetch_row($rsLastDt);
				$LastDate=$rowLastDt[0];
				//$LastDate="2017-07-15";
				$now = time(); // Current Date time
		...
		if (strpos($SelectedQuarter, 'Q3') !== false) 
			{
				$rsLastDt=mysqli_query($Con, "select FeesSubmissionLastDate from fees_master where Quarter='Q3'  and `Month`='$SelectedMonth' and financialyear='$FeeCollectionFY' limit 1");
				$rowLastDt=mysqli_fetch_row($rsLastDt);
				$LastDate=$rowLastDt[0];
				//$LastDate="2017-10-15";
				$now = time(); // Current Date time
		...
		if (strpos($SelectedQuarter, 'Q4') !== false) 
			{
				$rsLastDt=mysqli_query($Con, "select FeesSubmissionLastDate from fees_master where Quarter='Q4'  and `Month`='$SelectedMonth'and financialyear='$FeeCollectionFY' limit 1");
				$rowLastDt=mysqli_fetch_row($rsLastDt);
				$LastDate=$rowLastDt[0];
				//$LastDate="2018-01-15";
				$now = time(); // Current Date time
			...
			{
				$rsLastDt=mysqli_query($Con, "select FeesSubmissionLastDate from fees_master where Quarter='Q5'  and `Month`='$SelectedMonth' and financialyear='$FeeCollectionFY' limit 1");
				$rowLastDt=mysqli_fetch_row($rsLastDt);
				$LastDate=$rowLastDt[0];
				//$LastDate="2018-01-15";
				$now = time(); // Current Date time
				...
				$LateFee =$LateFee+ $TotalLateDays*10;
			}
				if (strpos($SelectedQuarter, 'Q6') !== false) 
			{
				$rsLastDt=mysqli_query($Con, "select FeesSubmissionLastDate from fees_master where Quarter='Q6'  and `Month`='$SelectedMonth' and financialyear='$FeeCollectionFY' limit 1");
				$rowLastDt=mysqli_fetch_row($rsLastDt);
				$LastDate=$rowLastDt[0];
				//$LastDate="2018-01-15";
				$now = time(); // Current Date time
	...
	//*************Late Fee Calculation End here*********
		      ...
		      $rsQ1Bounce=mysqli_query($Con, "select * from `fees` where `sadmission`='$sadmission' and `quarter` LIKE '%$ListOfQuarter%'  AND  FeeMonth  LIKE '%$SelectedMonth%' and `FinancialYear`='$FeeCollectionFY' and `cheque_status`='Bounce'");
		if(mysqli_num_rows($rsQ1Bounce)>0)
			$FeeBounceInQ1="yes";
		$rsQ2Bounce=mysqli_query($Con, "select * from `fees` where `sadmission`='$sadmission' and `quarter` LIKE '%$ListOfQuarter%'  AND  FeeMonth  LIKE '%$SelectedMonth%' and `FinancialYear`='$FeeCollectionFY'and `cheque_status`='Bounce'");
		if(mysqli_num_rows($rsQ2Bounce)>0)
			$FeeBounceInQ2="yes";
		$rsQ3Bounce=mysqli_query($Con, "select * from `fees` where `sadmission`='$sadmission' and `quarter` LIKE '%$ListOfQuarter%' AND  FeeMonth  LIKE '%$SelectedMonth%' and `FinancialYear`='$FeeCollectionFY'and  `cheque_status`='Bounce'");
		if(mysqli_num_rows($rsQ3Bounce)>0)
			$FeeBounceInQ3="yes";
		$rsQ4Bounce=mysqli_query($Con, "select * from `fees` where `sadmission`='$sadmission' and`quarter` LIKE '%$ListOfQuarter%' AND  FeeMonth  LIKE '%$SelectedMonth%' and `FinancialYear`='$FeeCollectionFY' and `cheque_status`='Bounce'");
		if(mysqli_num_rows($rsQ4Bounce)>0)
		$FeeBounceInQ4="yes";
				$rsQ5Bounce=mysqli_query($Con, "select * from `fees` where `sadmission`='$sadmission' and`quarter` LIKE '%$ListOfQuarter%' AND  FeeMonth  LIKE '%$SelectedMonth%' and `FinancialYear`='$FeeCollectionFY' and `cheque_status`='Bounce'");
		if(mysqli_num_rows($rsQ5Bounce)>0)
			$FeeBounceInQ5="yes";
			$rsQ6Bounce=mysqli_query($Con, "select * from `fees` where `sadmission`='$sadmission' and`quarter` LIKE '%$ListOfQuarter%' AND  FeeMonth  LIKE '%$SelectedMonth%' and `FinancialYear`='$FeeCollectionFY' and `cheque_status`='Bounce'");
		if(mysqli_num_rows($rsQ6Bounce)>0)
			$FeeBounceInQ6="yes";

		
	//echo "select distinct `feeshead` from `fees_student` where `sadmission`='$sadmission' and `financialyear`='$FeeCollectionFY' ORDER BY `feeshead` ";
	//exit();
...
$rsQ1=mysqli_query($Con, "select * from `fees` where `sadmission`='$sadmission' and `quarter` like '%Q1%'  AND  FeeMonth  LIKE '%$SelectedMonth%' and `cheque_status`!='Bounce' and `FinancialYear`='$FeeCollectionFY'");
if(mysqli_num_rows($rsQ1)>0)
	$FeePaidInQ1="yes";
$rsQ2=mysqli_query($Con, "select * from `fees` where `sadmission`='$sadmission' and `quarter` like '%Q2%' AND  FeeMonth  LIKE '%$SelectedMonth%' and `cheque_status`!='Bounce' and `FinancialYear`='$FeeCollectionFY'");
if(mysqli_num_rows($rsQ2)>0)
	$FeePaidInQ2="yes";
$rsQ3=mysqli_query($Con, "select * from `fees` where `sadmission`='$sadmission' and `quarter` like '%Q3%' AND  FeeMonth  LIKE '%$SelectedMonth%' and `cheque_status`!='Bounce' and `FinancialYear`='$FeeCollectionFY'");
if(mysqli_num_rows($rsQ3)>0)
	$FeePaidInQ3="yes";
$rsQ4=mysqli_query($Con, "select * from `fees` where `sadmission`='$sadmission' and `quarter` like '%Q4%' AND  FeeMonth  LIKE '%$SelectedMonth%' and `cheque_status`!='Bounce' and `FinancialYear`='$FeeCollectionFY'");
if(mysqli_num_rows($rsQ4)>0)
	$FeePaidInQ4="yes";
	
$FeePaidYesNo="no";
$rspaid=mysqli_query($Con, "select * from `fees` where `sadmission`='$sadmission' and `quarter`='$SelectedQuarter'  AND  FeeMonth ='$SelectedMonth' and `cheque_status`!='Bounce' and `FinancialYear`='$FeeCollectionFY'");
if(mysqli_num_rows($rspaid)>0)
	$FeePaidYesNo="yes";
...
<?php
	...
	while($rowFeeHead1=mysqli_fetch_row($rsFee1))
	{
		...
		//echo $TotalFeeHeadPreviousBalance;
		//exit();
		if($SelectedQuarter!="Q5")
		{
		//echo "select distinct `feeshead`,sum(`head_original_amount`),sum(`head_concession_amount`),sum(`amount`) from `fees_student` where `sadmission`='$sadmission' and `feeshead`='$feeshead' and `Quarter`='$SelectedQuarter'  AND  `Month`='$SelectedMonth' and `financialyear`='$FeeCollectionFY' group by `feeshead`"."<br>";
		$rsFee=mysqli_query($Con, "select distinct `feeshead`,sum(`head_original_amount`),sum(`head_concession_amount`),sum(`amount`) from `fees_student` where `sadmission`='$sadmission' and `feeshead`='$feeshead' and `Quarter`='$SelectedQuarter'  AND  `Month`='$SelectedMonth' and `financialyear`='$FeeCollectionFY' group by `feeshead`");
...
<?php
//echo $HeadWiseAmountTotal."/".$LateFee."/".$ChequeBounceCharges;
//exit();
...
{
	var myWindow = window.open("ShowReceiptMonthly.php?Receipt=" + receiptno ,"","width=700,height=650");	
}
...
<div class="page-container">
							...
							<div class="table-container">
								...
								<tr>
									...
									<td>
										...
										<?php echo $SelectedQuarter;?> -<?php echo $SelectedMonth;?>
																<br>	<b><a href="http://nkbpsdsis.mobilisesis.co.in/Admin/Fees/DisplayClassWiseChallan_monthly.php?txtAdmissionNo=<?php echo $sadmission;?>&Class=<?php echo $sclass;?>&txtQuarter=<?php echo $SelectedQuarter;?>&txtMonth=<?php echo $SelectedMonth;?>&txtFY=<?php echo $FeeCollectionFY;?>" target="_blank"><input type=button class="btn red" value="View Challan"></a></b><br>
	...
	<form name="frmFees" id="frmFees" method="post" action="FeePaymentPayU_monthly.php">
								...
								View Quarter Wise Fees Detail</span>
								   ...
								   <input type="hidden" id="txtAdmissionNo" name="txtAdmissionNo" value="<?php echo $sadmission;?>">
								             <input type="hidden" id="cboQuarter" name="cboQuarter" value="<?php echo $SelectedQuarter;?>">
								              <input type="hidden" id="cboMonth" name="cboMonth" value="<?php echo $SelectedMonth;?>">
...
?>
							...
							?>
								...
								<table class="table table-striped table-bordered table-hover" >
	<tr>
		<td style="border-style:solid; border-width:1px; height: 16px; " colspan="9" align="center" >
	...
	<tr>
		...
		<font face="Cambria" style="font-size: 10pt"><strong>Fee Payable</strong></font></th>
		<th style="border-style:solid; border-width:1px; height: 16px; width: 123px" align="center" >
		<font face="Cambria" style="font-size: 10pt"><strong>Fee Paid</strong></font></th>
		<th style="border-style:solid; border-width:1px; height: 16px; width: 123px" align="center" >
		<font face="Cambria" style="font-size: 10pt"><strong>Balance</strong></font></th>
		<th style="border-style:solid; border-width:1px; height: 16px; width: 123px" align="center" >
		<font face="Cambria" style="font-size: 10pt"><strong>Status</strong></font></th>
		<th style="border-style:solid; border-width:1px; height: 16px; width: 123px" align="center" >
		<font face="Cambria" style="font-size: 10pt"><strong>Payment Date</strong></font></th>
		<th style="border-style:solid; border-width:1px; height: 16px; width: 123px" align="center" >
		<font face="Cambria" style="font-size: 10pt"><strong>Financial Year</strong></font></th>
		<th style="border-style:solid; border-width:1px; height: 16px; width: 123px" align="center" >
...
<?php echo $amountpaid; ?>
		...
		<?php if($ChequeStatus!="Bounce"){echo $status;} else {echo $ChequeStatus;} ?>
...
<?php
	...
	$ssql = "SELECT `quarter` FROM `fees` where `sadmission`='$sadmission'  order by `datetime` desc ";
	$rs = mysqli_query($Con, $ssql);
...
while($row = mysqli_fetch_row($rs))
	{
					...
					if($quarter == 'Q4')
					{
					    ?>
					    <p align="right">
                        <a href="FeeCertificate.php" target="_blank"><button type="button" class="btn yellow">Download Fees Certificate</button></a>
                        </p>
<?php
					}
...
<br><br>
<a href="MyFees_prev_year.php"><button class="btn btn-danger" style="height:50px;">Click here to download the previous fees certificate</button></a>
