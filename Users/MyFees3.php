...
$rsFeeHeadAmt=mysqli_query($Con, "SELECT distinct `feeshead`,sum(`amount`) FROM `fees_student` WHERE `sadmission`='$sadmission' and `FeesType` !='Hostel' and `FeesType` !='' and `feeshead` not in ('COMPUTERFEES','SMART CLASS','SCIENCE FEES','ANNUAL CHARGES') group by `feeshead`");
		...
		$rsYearlyConcessionAmt=mysqli_query($Con, "SELECT `amount` FROM `fees_student` WHERE `sadmission`='$sadmission' and `FeesType` !='Hostel' and `feeshead`='Total Concession Amount'");
		...
		$rsAdvanceAmt=mysqli_query($Con, "select sum(`amount`) from `fees_student` where `sadmission`='$sadmission' and `feeshead` in ('Advances')");
		...
		//CHECK FEE SUBMISSION IN FEE TABLE FOR QUARTER Q3 IF FOUND THEN SHOW RECEIPT DETAIL OTHER WISE TO PAID AMOUNT WILL BE SHOWN//////////////////
			...
			else
			{
				...
				{
					$rsRcptNo=mysqli_query($Con, "select `receipt`,DATE_FORMAT(`date`,'%d-%m-%Y') as `date` from `fees` where `sadmission`='$sadmission' and `quarter`='Q3'  and `FeesType` !='Hostel'");
					while($rowRcpt=mysqli_fetch_row($rsRcptNo))
					{
		...
		///HOSTEL AMOUNT CALCULATION**************************
						...
						$rsHostelPaidAmt=mysqli_query($Con, "select sum(`amountpaid`) from `fees` where `sadmission`='$sadmission' and `FinancialYear`='$FeeSubmissionFinancialYear' and `FeesType` ='Hostel'");
		...
		while($rowFPQ4=mysqli_fetch_row($rsTotalHostelAmt))
		{
			...
			//$FeeAmtPaidQ4=($YearlyTotalFeeAmount-($FeeAmtPaidQ1+$FeeAmtPaidQ2+$FeeAmtPaidQ3+$ToalHostelAmtPayable+$FeeAmtPaid+$TotalConcessionAmt));	
			  
			  //$FeeAmtPaidQ4=($YearlyTotalFeeAmount-($FeeAmtPaidQ1+$FeeAmtPaidQ2+$FeeAmtPaidQ3+$FeeAmtPaid));	
			  
			//echo $YearlyTotalFeeAmount."/".$TotalAmountPaid."/".$FeeAmtPaidQ1."/".$FeeAmtPaidQ2."/".$FeeAmtPaidQ3;
			//exit();
