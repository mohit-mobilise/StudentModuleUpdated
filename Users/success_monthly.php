...
If (isset($_POST["additionalCharges"])) 
{
	   ...
	   else 
	   {
				   	...
				   	//exit();	
						...
						$ssql="UPDATE `fees_temp` SET `TxnStatus`='$status',`PGTxnId`='$pgtxnno' where `TxnId`='$txnid'";
						mysqli_query($Con, $ssql) or die(mysqli_error($Con));
						$ssql="UPDATE `fees_transaction_temp` SET `TxnStatus`='$status',`PGTxnId`='$pgtxnno' where `TxnId`='$txnid'";
						mysqli_query($Con, $ssql) or die(mysqli_error($Con));
					...
					while($rows = mysqli_fetch_row($rsSchoolDetail))
					{
						...
						$currentYear=date("Y");
							...
							$ssql="INSERT INTO `fees` (`sadmission` ,`sname` ,`sclass`,`srollno`,`fees_amount`,`AdjustedLateFee`,`DiscountAmount`,`amountpaid`,`BalanceAmt`,`quarter`,`date`,`FinancialYear`,`status`,`receipt`,`finalamount`,`ReceiptFileName`,`PaymentMode`,`chequeno`,`bankname`,`branch`,`cheque_date`,`cheque_status`,`DebitHead`,`TxnAmount`,`TxnId`,`PGTxnId`,`TxnStatus`,`ActualLateFee`,`cheque_bounce_amt`,`FeeMonth`) select `sadmission` ,`sname` ,`sclass`,`srollno`,`fees_amount`,`AdjustedLateFee`,`DiscountAmount`,`amountpaid`,`BalanceAmt`,`quarter`,`date`,`FinancialYear`,`status`,`receipt`,`finalamount`,`ReceiptFileName`,`PaymentMode`,`chequeno`,`bankname`,`branch`,`cheque_date`,`cheque_status`,`DebitHead`,`TxnAmount`,`TxnId`,`PGTxnId`,`TxnStatus`,`ActualLateFee`,`cheque_bounce_amt`,`FeeMonth` from `fees_temp` where `TxnId`='$txnid'";
							mysqli_query($Con, $ssql) or die(mysqli_error($Con));
			
							$ssql1="INSERT INTO `fees_transaction` (`feeshead`,`headamount`,`sadmission`,`sname`,`ReceiptNo`,`ReceiptDate`,`ActualLateFee`,`ActualDelayDays`,`AdjustedLateFee`,`AdjustedDelayDays`,`DiscountAmount`,`Remarks`,`FinancialYear`,`finalamount`,`FeeMonth`,`FeeYear`,`TxnAmount`,`TxnId`,`PGTxnId`,`TxnStatus`,`quarter`,`PaidAmount`,`PaymentMode`,`cheque_bounce_amt`) select `feeshead`,`headamount`,`sadmission`,`sname`,`ReceiptNo`,`ReceiptDate`,`ActualLateFee`,`ActualDelayDays`,`AdjustedLateFee`,`AdjustedDelayDays`,`DiscountAmount`,`Remarks`,`FinancialYear`,`finalamount`,'$FeeFor',`FeeYear`,`TxnAmount`,`TxnId`,`PGTxnId`,`TxnStatus`,`quarter`,`PaidAmount`,`PaymentMode`,`cheque_bounce_amt` from `fees_transaction_temp` where `TxnId`='$txnid'";
...

{
...

	{
		...
		toastr.warning("Please enter student addmission id", "Validation Error");
		return;
...
<div id="divPrint">
	...
	<a href="Javascript:printDiv();"><span >PRINT</span></a> || <a href="FetchStudentDetail.php"><span >HOME</span></a>
