...
$is_fees_access = $rsFeeTab['is_fees_access'] ?? '0';
$is_hostel_fee_access = $rsFeeTab['is_hostel_fee_access'] ?? '0';
...
{
  global $Con; // Make sure $Con is accessible in the function
...
function fnlLateFee($month,$class,$adm,$year,$lateFeeCheckCalculation=0,$DiscontType)
{
  ...
  global $lateFeeCalculationType, $Con; // Make sure $Con is accessible in the function
			...
			<div class="table-responsive">
               ...
               <?php
                ...
                //             $sqlfindmonth = mysqli_query($Con, "SELECT * FROM `fees_transaction` as `a` LEFT JOIN `fees` as `b` on (`a`.`sadmission`= `b`.`sadmission` and `a`.`receipt_no`= `b`.`receipt_no`) WHERE `a`.`sadmission`='$sadmission'  and `a`.`Month`='$Month' and `a`.`fy`='$CurrentFY' and `a`.`status`='1' and `b`.`cheque_status` in ('Clear','Online')");
                            ...
                            $latefee = fnlLateFee($Month,$masterclass,$sadmission,$CurrentFY,$lateFeeCheckCalculation,$DiscontType);
                            ...
                            $head_Listing = $rsamt['head_id']; 
                                $headIdArray[]=$head_Listing;
                    ...
                    <tr>
                    	 <td style="width: 10px;">
                            ...
                            <input type="checkbox" class="getamt" name="mycheckbox[]" id="<?= "chkcheckbox_".$cnt; ?>" value="<?= $cnt; ?>" <?php if($is_fee_month_show == '1' || $is_fee_month_show == 1) { ?> checked <?php } ?> >
                      ...
                      <table class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
                          ...
                          <?php
                              ...
                              $hstlcnt =1;
                              if ($sqlHostelMonth && is_resource($sqlHostelMonth)) {
                                  mysqli_data_seek($sqlHostelMonth, 0);
                              }
                          ...
                          <?php    
                            ...
                            if ($sqlhostelfeehead && is_resource($sqlhostelfeehead)) {
                                mysqli_data_seek($sqlhostelfeehead, 0);
                            }
       ...
       <input type="hidden" name="challan_idtnfy" id="challan_idtnfy">
           ...
           <div class="row">
            ...
            <div class="row mt-3">
               ...
               <table>
                  <?php
                       if ($sqlMonth && $sqlMonth !== '' && (is_resource($sqlMonth) || is_object($sqlMonth))) {
                           mysqli_data_seek($sqlMonth, 0);
                         ...
                         <?php
                           ...
                           echo "<tr><td colspan='2'>No months available</td></tr>";
                       }
                  ?>
              </table>
           </div>
